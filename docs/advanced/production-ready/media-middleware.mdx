...
`dangerouslyDisableLazyLoad` or (even better in the following example) **the
`priority` prop that will also add a `<link rel="preload">` to the page.**
...
article. [The "Images size" section below](#image-sizes) explains how we added
`sizes` attributes in our themes, and how and when you should override it. Here
is an example of the `sizes` attribute:
...
And let's further assume that the image sizes available are [68, 136, 272, 544].
...
- [adding custom endpoints to the Node.js server](/docs/advanced/server/add-http-endpoint)
  (with express Router)
...
export const mediaProxyRouter = () => {
  ...
  router.use(
    ...
    makeImageProxyRouter((transformImageBuffer) => {
      ...
      // see https://developers.front-commerce.com/docs/advanced/server/configurations for further details
...
### Ignore caching for builded files
