...
follow the same
[semantic versioning](http://localhost:3002/docs/appendices/release-process#semantic-versioning)
as the core.
...
`6.2.1`. If you used this import without installing it in your project, the
...
1. the `FRONT_COMMERCE_ADYEN_CLIENT_KEY` environment variable is now required so
   you have
   [to configure the environment so that it is defined](/docs/advanced/payments/adyen#add-the-required-environment-variables-1)
