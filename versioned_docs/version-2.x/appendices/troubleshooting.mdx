...
> In Dev mode, I see
> [the SSR Fallback page](/docs/2.x/advanced/theme/server-side-rendering#ssr-fallback-when-things-go-wrong)

1. check the stdout/stderr of your server and the `/logs` folder, there may be
   useful information
2. are all pages broken or only some of them?
3. ensure that there is
   [no code using browser APIs loaded server side](/docs/2.x/advanced/theme/server-side-rendering#there-is-no-window-on-the-server)(e.g:
   Google Maps)
...
[configuration provider](/docs/2.x/advanced/server/configurations).
...
URL with the
[`DEBUG="front-commerce:config"` environment variable](/docs/2.x/reference/environment-variables#debugging).
...
[registered in your application](/docs/2.x/advanced/server/configurations#overriding-express-configuration-provider)
as any other one.
...
[introduced in Front-Commerce 2.3](/docs/2.x/appendices/migration-guides/archives#elasticsearch-7x-and-elasticsuite-29)_
...
[an external Search Engine](/docs/2.x/magento2/search-engine), and with
