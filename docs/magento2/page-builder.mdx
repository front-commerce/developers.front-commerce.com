...
dynamicically sized images, or use a different format.

You can register your own image content type, and use a custom format, or you
can determine the format dynamicically using a class.
...
const DESKTOP_IMAGE_FORMAT = "large";
const MOBILE_IMAGE_FORMAT = "medium";
...
![Default Formats](./assets/pb-image-ct-default.png)

</BrowserWindow>

### Determine format dynamically

With this method you can use the classes from the page builder to detrime the
...
When then need to register the `DynamicImage` content-type with the PageBuilder
...
![Dynamic Formats](./assets/pb-image-ct-dynamic.png)
...
```js title="src/server/modules/page-builder/loaders/index.js"
import Image from "server/magento2/wysiwyg/loaders/content-types/Image";
import { ORIGINAL_PRESET_CODE } from "server/core/image/imageResizer"; // since version: 2.18.0, otherwise defaults to "original"
...
![Original Format](./assets/pb-image-ct-original.png)
