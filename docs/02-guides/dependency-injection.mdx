...
interface from `@front-commerce/types` to include your custom dependencies. This
declaration can be placed in a `dependency-injection.d.ts` file within your
extension's types directory.

Ensure you have the `@front-commerce/types` package installed in your project,
as it provides the necessary types for DI.
