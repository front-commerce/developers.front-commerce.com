...
[Routing documentation](/docs/2.x/reference/routing)). If the URL matches one of
the routes available here, it will preload the component and then preload its
data using the `preload` static property of the component.
...
[`preload` property in your `config/website.js`](/docs/2.x/reference/configurations#configwebsitejs).
