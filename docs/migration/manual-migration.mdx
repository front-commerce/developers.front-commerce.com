...
.example {
  // remove-start
  padding: var(--boxSizeMargin) * 2 var(--boxSizeMargin) * 4
    var(--boxSizeMargin) * 2 0;
  // remove-end
  // add-start
  padding: calc(var(--boxSizeMargin) * 2) calc(var(--boxSizeMargin) * 4)
    calc(var(--boxSizeMargin) * 2) 0;
  // add-end
...
- The `registerNodesFetcher` method will no longer work in v3, instead you
  should
  [register a fetchers](/docs/3.x/guides/customize-the-sitemap#creating-and-registering-custom-fetchers)
  via the
...
- The `overrideNodesFetcher` method will no longer work in v3, instead you
  should
  [override a fetcher](/docs/3.x/guides/customize-the-sitemap#creating-and-registering-custom-fetchers)
  via the
  [sitemap service](/docs/3.x/api-reference/front-commerce-core/services#server-services)
...
## Error Pages Migration

<SinceVersion tag="3.8" />

In v2 static error pages was defined in
[`PageError`](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/254a2aa06ae519d550d437253a81def63c385f5d/theme-chocolatine/web/theme/modules/PageError/PageError.js)
and
[`NotFound`](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/254a2aa06ae519d550d437253a81def63c385f5d/theme-chocolatine/web/theme/modules/PageError/NotFound/NotFound.js)
components

In v3, we have added a more robust way of routing errors via the the
[`appErrorPages.ts`](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/main/packages/theme-chocolatine/theme/pages/Error/appErrorPages.ts?ref_type=heads)
config file. Please follow our
[Customize Error Pages](/docs/3.x/guides/error-handling-for-routes#customizing-error-pages)
guide to see how you can implement custom error pages.
