...
[`modules` entry from the `.front-commerce.js` file](/docs/reference/front-commerce-js#modules).
Each module can contain client code (See
[Extend the theme](/docs/essentials/extend-the-theme)), and server code (current
guide).
...
[Extend the GraphQL Schema](/docs/essentials/extend-the-graphql-schema) for more
information.
