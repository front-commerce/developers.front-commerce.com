...
When using Front-Commerce with Magento 1 as backend you can set the env variable
`FRONT_COMMERCE_MAGENTO_TYPE` to `magento1` or `openmage` to specify to Mangeto
1 backend type (by default `openmage`).
...
`theme/modules/StorefrontContent/sources`, in that example:
...
export default [magento, contentful, prismic, aCustomSource].filter(Boolean);
```
