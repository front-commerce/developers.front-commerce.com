...
the components used for the feature. All the components are merged together, so
not including a component, it will still include the component from the origin
unless overriding it with `null`.
