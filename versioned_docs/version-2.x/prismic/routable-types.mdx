...
description:
  You might want to create new pages managed from Prismic. These pages should be
  accessible with a URL and may have to be included in the Sitemap with a clean
  URL. This guide explains everything that is supported by Front-Commerce's
  Prismic module.
---

<p>{frontMatter.description}</p>

The Prismic module allows to leverage
[Front-Commerce's Dispatcher](/docs/2.x/advanced/theme/route-dispatcher) to make
content from custom types routable.

## Create a field to represent the Url of the custom type

To create a custom field to represent the Url of the custom type:

1. Head over to the Prismic console and navigate to custom types
   (`https://<prismic_repository_name>.prismic.io/masks/`)
1. Select the custom type to make routable
1. Drag and drop `Key Text - Text field for exact match search` from the right
   to the Main tab on the left.
1. Give the new field a `Field name`, `API ID` and optionally a
   `Field placeholder` (note the API ID is an important field and will be used
   below)
...
If no GraphQL type was created to represent the custom data type within
Front-Commerce, please create a GraphQL type to represent the custom type in
schema.gql of the server module's directory
...
[Add GraphQL type to the dispatcher query section of the route dispatcher documentation](/docs/2.x/advanced/theme/route-dispatcher#add-graphql-type-to-the-dispatcher-query)
...
after
[the transformation is done using `contentTransformOptions`](/docs/2.x/prismic/expose-content#field-transformers).
...
[Add the mapping between the type and the page component](/docs/2.x/advanced/theme/route-dispatcher#add-the-mapping-between-the-__typename-and-the-page-component)
in the [route dispatcher](/docs/2.x/advanced/theme/route-dispatcher).
...
Optionally to add the custom Prismic type to the sitemap use the following
steps:
...
```diff title="my-module/resolvers.js"
export default {
Query: {
...
},
...
Registers a routable type within Front-Commerce and adds the route to the
Prismic Route Resolver, it can also create a sitemapable route.
...
- `path` (string): the dynamic route. Examples: `/:uid`, `/:lang/:uid`,
  `/:section/:category?/:uid`.
- `rewrites` (string[]) : the rewrite paths to apply on a url, they will be
...
To leverage the
[Prismic Route Resolver](https://prismic.io/docs/core-concepts/link-resolver-route-resolver#route-resolver),
...
It's also important to note that only one route for a document type can be added
to the route resolver, the last registered route will override the previous
route e.g.
...
> This method will not create a resolvable route in Front-Commerce. If you need
> a routable type please use the `registerRoutableType` method instead, it also
> registers a prismic route.
...
As you may notice the relationships fields can continue infinitely deep, but as
mentioned they are limited to the second level.
...
Here are some examples of how the paths would be resolved for this registered
route.

```js title="Example 1: with existing relationships"
...
```js title="Example 2: without existing relationships"
