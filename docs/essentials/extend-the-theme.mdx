...
description:
  Theme override is the mechanism allowing to extend, adapt, and rewrite almost
  everything contained in the base theme.
---
...
most of the time you might find easier and faster to start with the base theme
...
[themes in Magento](https://devdocs.magento.com/guides/v2.3/frontend-dev-guide/templates/template-overview.html),
[child themes in Wordpress](https://developer.wordpress.org/themes/advanced-topics/child-themes/)
/
[Prestashop](https://devdocs.prestashop.com/1.7/themes/reference/template-inheritance/parent-child-feature/),
[templates override in Drupal](https://www.drupal.org/docs/8/theming/twig/working-with-twig-templates),
...
from parent theme(s) — at least from Front-Commerce base theme. You would then
copy the files you want to override in your theme folder by maintaining an
identical file structure. Your component will then be used instead of the one in
your parent theme(s).

This translates in those three steps:

1. configure your custom theme and use it in your application
2. copy the file (`js`, `scss` or `gql`) you want to override in the `theme`
...
First, we need to create a minimalist module that will contain your own theme by
creating the following file structure (for instance at the root of your
project):
...
[`.front-commerce.js`](/docs/reference/front-commerce-js) file. If the module
has been created at the root of your project, it would look like this:
...
Restart the application. That’s it! You have successfully registered the module
...
Please refer to the
[Front-Commerce’s folder structure documentation page](/docs/concepts/front-commerce-folder-structure)
...
2. add the description after the `<ProductOverview />` component in your
   `ProductItem` with `{this.props.description}`.
...
1. add the field `description` to the fragment.
   ...
   ```diff title="my-module/web/theme/modules/ProductView/ProductItem/ProductItemFragment.gql"
...
In development mode, you MIGHT need to restart the application for the override
...
It is possible to import a component from the `front-commerce` node module in a
