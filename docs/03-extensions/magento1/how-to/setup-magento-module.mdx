...
`FRONT_COMMERCE_MAGENTO1_CONSUMER_KEY` and
`FRONT_COMMERCE_MAGENTO1_CONSUMER_SECRET` environment variables in your `.env`
file.
...
- Click on "Generate Token" link in installer checker section (This should be
  the same key as `FRONT_COMMERCE_MAGENTO1_ACCESS_TOKEN` and
  `FRONT_COMMERCE_MAGENTO1_ACCESS_TOKEN_SECRET` in your Front-Commerce `.env`)
