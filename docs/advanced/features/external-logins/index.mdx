...
> _Since version 2.13 (Beta feature ~ API may change)_
...
you are have the magento1 connector on 1.4.2 or later.
...
folowing environment variables:
...
External login rely on redirections, and in devlopment your server is usally at
`http://localhost` i.e. not reachable to Facebook. So to circumvent this in
...
service will provide a publically accessible URL to your local server. So to
work with Facebook external logins on your local machine you need to set the
...
Please refer to
[Working with the Facebook provider in development mode](#Working-with-the-Facebook-provider-in-development-mode)
