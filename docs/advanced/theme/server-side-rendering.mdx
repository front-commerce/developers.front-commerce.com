...
[Front-Commerce's configuration system](/docs/advanced/server/configurations) to
provide information about the device. Each `req.config` will contain a `device`
...
[Server timings](/docs/advanced/performance/server-timings) could help you to
spot performance issues, read the
...
[how to disable it in Front-Commerce](/docs/advanced/features/disable-hyration).
