...
In this version we fixed the add to wishlist state updates after the first
registration. Previously it would not display a product in the wishlist until a
full page reload is initiated.

To accommodate this fix, we had to add changes to the following files;

- [src/web/theme/modules/Wishlist/AddProductToWishlist/withIsProductInWishlist.js](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/merge_requests/3399/diffs#fc726e322f9602537264acadd3e6b052763e73e5)
- [src/web/theme/modules/Wishlist/AddProductToWishlist/withProductWishlistMutations.js](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/merge_requests/3399/diffs#9c0961f0b27e34868027c7e97f7c5ed14f4a2695)

If you have overridden these files please ensure to update them with the
changes.

### Reworked QuickOrder component

In this version, we've introduced a new feature that enables you to bulk import
products from a CSV file and directly add them to your cart.

:::info The quick order page is active by default

The Quick order page is active by default at `/quick-order`. If you want to
opt-out of this feature, you will have to remove the route
[using the `front-commerce-prepare.js::onCreateRoute` callback](/docs/2.x/reference/front-commerce-prepare#oncreateroute).

:::

To develop this new feature, we've had to update the following files;

- [`src/web/theme/modules/ProductPicker/ProductPicker.js`](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/f2efb127286d4f17a5fb6d48d23fb32c0418e9be#ca297fed67eb602d111f743cf4a14b155f050366)
- [`src/web/theme/modules/AddToCart/AddToCartModal/AddToCartModalQuery.gql`](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/f2efb127286d4f17a5fb6d48d23fb32c0418e9be#ca297fed67eb602d111f743cf4a14b155f050366)
...
triggered multiple times by refreshing the `/checkout/success` page (see
[related commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/a5391ed51fdaa03d55e2dada638a6d0777697712)).

To benefit from this fix, you will need to ensure that the following changes
have been applied to your project, in the case you overrode those files:

```diff title="theme/modules/Checkout/withCheckoutTracking/withCheckoutSuccessTracking.js"
...
```diff title="theme/pages/Checkout/stepsDefinition.js"
...
- [a new `onServerInit` hook to extend the Express server](/docs/2.x/advanced/server/add-http-endpoint#extend-the-http-server-with-onserverinit)
...
In version 2.26.0, we have updated the behavior of the requisition list UID to
align with the Magento 2 GraphQL API.

The requisition list UID is now a base64-encoded string instead of a decoded
string value.

If you are using a Magento version lower than 2.4.5, you must update the
requisition list module by running the following command:
...
We have added the
`FRONT_COMMERCE_MAGENTO_UNSAFE_LEGACY_REQUISITION_LIST_ID=true` environment
...
In this version, we have added the ability to configure the timeouts for the
Axios instance used by Front-Commerce. We recommend using the new configuration
instead of the
[timeout file with static values](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/bb9bda623701402156566e2e408a9afe6d7a9405/src/server/core/graphql/timeout.js),
which has been marked as deprecated for v3.0.

The loaders that previously used these values have been updated to use a
`configProvider` to get the values, which are customizable via the following
environment variables:

- [Magento1 configProvider](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/bb9bda623701402156566e2e408a9afe6d7a9405/src/server/modules/magento1/core/config/magentoProvider.js#L70-81)
  - `FRONT_COMMERCE_MAGENTO_TIMEOUT` (default: 60000)
  - `FRONT_COMMERCE_MAGENTO_ADMIN_TIMEOUT` (default: 10000)
- [Magento2 configProvider](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/bb9bda623701402156566e2e408a9afe6d7a9405/src/server/modules/magento2/core/config/magentoProvider.js#L149-160)
  - `FRONT_COMMERCE_MAGENTO_TIMEOUT` (default: 60000)
  - `FRONT_COMMERCE_MAGENTO_ADMIN_TIMEOUT` (default: 10000)
- [Colissimo configProvider](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/bb9bda623701402156566e2e408a9afe6d7a9405/modules/shipping-colissimo-magento2/server/modules/colissimo/config/colissimoConfigProvider.js#L5-10)
  - `FRONT_COMMERCE_COLISSIMO_TIMEOUT` (default: 10000)
- [Ogone configuration](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/bb9bda623701402156566e2e408a9afe6d7a9405/src/server/modules/payment-ogone/makeConfigFromEnv.js#L74)
  - `FRONT_COMMERCE_OGONE_TIMEOUT` (default: 10000)

We hope this documentation helps you configure the timeouts for the Axios
...
- **payment**: Authorize.net is now supported for Magento1 (OpenMage LTS)
  projects. To use it, please update the Magento module
  [`front-commerce/magento1-module` to 1.5.0+](https://gitlab.blackswift.cloud/front-commerce/magento1-module-front-commerce/-/releases/1.5.0)
...
`QueryHelper.filterCanonicalQueryStringParameters` compute the canonical URL
parameters, in order to better fit SEO-recommended rules (see
[Google article](https://developers.google.com/search/docs/crawling-indexing/canonicalization#:~:text=the%20results%20of%20sorting%20and%20filtering%20functions%20of%20a%20category%20page)
on this matter).

To opt-in to this behaviour, you can pass a boolean as the second parameter of
`QueryHelper.filterCanonicalQueryStringParameters` to configure whether or not
additional filters should be included in the canonical. `true` (default value)
will include additional filters, and `false` will exclude them. Please note that
we removed the `sorting`, `minPrice` and `maxPrice` parameters from the
canonical url parameters.

See more in the
[related commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/ebffdc8412f2657bff6782bab1c49568fc78520a?merge_request_iid=2121).

### Drop support of Node 14

In this version we've **dropped support for Node 14**
(<abbr title="End Of Life">EOL</abbr>: 2023-04-30). The minimum required version
is now Node 16.
...
<ContactLink>Please let us know</ContactLink> if you encounter any issues
regarding this new behavior.
...
```graphql title="src/web/theme/modules/User/Order/OrderShipmentTracking/OrderShipmentTrackingFragment.gql"
// add-next-line
...
If you've overridden the `BaseTrackingDetails` component, you must add a new
component to display the list of items:

```js title="src/web/theme/modules/User/Order/OrderShipmentTracking/OrderShipmentTracking.js"
...
To implement the Magic Button related features, we have changed several files
from the theme:

- `theme/layouts/GenericLayout.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/0378fcceafbd7cb4bf5a1c26e7c9080cdd66fcf7))
- `theme/modules/ProductView/ProductItem/ProductItem.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e))
- `theme/modules/CmsBlock/CmsBlock.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e))
- `theme/pages/Product/Product.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e))
- `theme/pages/Account/Dashboard/Dashboard.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e))
- `theme/pages/Account/Dashboard/EnhanceDashboard.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e))
- `theme/pages/Account/Orders/Details/Invoice/Invoice.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e))
- `theme/pages/Account/Orders/Details/Details.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e))
- `theme/pages/CmsPage/CmsPage.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/61fddb8f9c68278a818a3875fb29c3c4294df4eb#0910701e610443ed0dabb3fb9b053e2b3b9e4ad7_22_13))
- `theme/pages/Category/Category.js`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/8234c24547b3a22a737f9279ebafddb697135a5e#f73633265fb675f84c088c5b3d38749242fe36f8))
- **Mandatory** `theme/components/atoms/Colors/_colors.scss`
  ([see the corresponding commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/d34b4bf4819fc2c8e4275d28aa62d94608aa418e#cb2e530ac7e57d50b7a4e6773fd6b24b86b0a982))
- `theme/modules/_modules.scss` (see
  [this first commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/d34b4bf4819fc2c8e4275d28aa62d94608aa418e?page=2#f9a39a6723f4100486c2658dff702698421eae41)
  and
  [this second commit](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/commit/5d803de8a30d9429cf4c085c3b3da91285c22ae1#f9a39a6723f4100486c2658dff702698421eae41))
- `theme/components/_components.scss`
  ([see the corresponding changes](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/merge_requests/2084/diffs#diff-content-1ff60716b8b194110c074a70473544b96850f2ac))

If you have an override of `template/index.html`, you need to apply the
following changes:

```diff title="template/index.html"
     <script>
...
[Focal Point](/docs/2.x/reference/image-component#focal-point-focalpoint)
support for the `<Image>` component.

**If you use
[Front-Commerce's media middleware](/docs/2.x/advanced/production-ready/media-middleware)**,
the cache needs to be fully regenerated.

To do so, you should delete the `.front-commerce/cache/images` directory and
deploy your project.

:::caution

Regenerate image cache will lead to a **high CPU usage**, to not disturb the
availability of your app, we recommend you do this outside peak visit time on
your website.
...
<>
  We've{" "}
  <a href="https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/merge_requests/2120">
    removed the chunks preloading feature
  </a>
  as it led to blocking network requests that could reduce the{" "}
  <abbr title="Largest Contentful Paint">LCP</abbr> /{" "}
  <abbr title="First Contentful Paint">FCP</abbr>
</>

of your app. From our analysis it improves the performance of the application
and is a good default.

We invite you to test the impact of this change (i.e: run a Core Web Vitals test
on your application and compare the results to confirm this).

If you want to restore the previous behavior, you can set the
`FRONT_COMMERCE_ENABLE_CHUNKS_PRELOAD=true` environment variable.
...
[2.24.1](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/releases/2.24.1)
which fixes a critical regression introduced in version 2.24.0. **Please ensure
that you're upgrading to the latest 2.24.x version.**
...
expose a `scopes.queries.matchingQuery` scope. You can use a dummy
implementation if you don't want to support search query autocompletion. See
[MR!2072](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/merge_requests/2072)
for an example.

If you use an existing Front-Commerce search engine (Platform native,
ElasticSearch, Algolia or Attraqt), you don't have anything to do.
...
- [Akamai Image Manager adapter](/docs/2.x/advanced/images-adapters/akamai-image-manager)
...
[`registerRoutableType`](/docs/2.x/prismic/routable-types#registerroutabletypeoptions)
method or the
[`registerPrismicRoute`](/docs/2.x/prismic/routable-types#registerprismicrouteoptions)
...
- [Search for products, categories and pages with Algolia in Magento2 based project](/docs/2.x/magento2/search-engine#algolia-connected-to-magento)
...
[the MondialRelay guide page](/docs/2.x/advanced/shipping/mondial-relay#integrate-with-magento1-openmagelts).
...
1. the Magento2 Adyen Front-Commerce module has been moved. As a result, you
   have to
   [update your `.front-commerce.js` as documented in the Adyen integration page](/docs/2.x/advanced/payments/adyen#register-the-adyen-payment-module-in-front-commerce).
1. the `FRONT_COMMERCE_ADYEN_CLIENT_KEY` environment variable is now required so
   you have
   [to configure the environment so that it is defined](/docs/2.x/advanced/payments/adyen#add-the-required-environment-variables-1)
