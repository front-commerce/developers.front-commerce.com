...
you want. See [WYSIWYG documentation](/docs/advanced/theme/wysiwyg) for more
details.
...
[Magento's WYSIWYG supported features](/docs/advanced/theme/wysiwyg#Magento-theme-modules-Wysiwyg-MagentoWysiwyg)
for more details.

### Load configuration files from parent modules

[Custom themes](/docs/essentials/extend-the-theme) defined in the
[`modules` key of your `.front-commerce.js` file](/docs/reference/front-commerce-js#modules)
...
- Auto redirect HTTP requests to HTTPS in production mode
  ([see more details](/docs/appendices/migration-guides#HTTPS))
- Improved schema stitching from developers feedbacks to allow custom headers
  and authenticated requests
  ([see more details](/docs/advanced/graphql/remote-schemas#Customize-remote-HTTP-requests))
