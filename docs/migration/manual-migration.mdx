...
- `isResolvedFromProps`, previously a function, parameter has been replaced with
  `hasResolved` which is now a boolean
- `mapPropsToProperties`, previously a function, has been renamed to
  `createProperties` but is otherwise unchanged
