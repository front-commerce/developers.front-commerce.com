...
<abbr title="Content Security Policy">CSP</abbr> in your [config/website.js
configuration](/docs/reference/configurations#configwebsitejs).

## Basic configuration

For security reasons only URLs from the store's domain are authorized through
CSP. However, for tracking and third-party dependencies, you will have to authorize more
domains. Use the following config and add your custom domain in each:
