...
Ideally, you would execute this in your loaders. However, for the clarity of
this guide, we're using the contextEnhancer.
...
```js title="./my-extension/cms/index.js"
      ...
      Query: {
        loadHomepages: async () => {
          const allPagesResponse =
            await loaders.Contentful.loadAllContentMatching(
              homepage,
              undefined,
              0,
              5
            );
          return allPagesResponse.items.map(homepage.dataFormatter);
        },
      },
    };
  },
