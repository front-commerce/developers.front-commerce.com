...
You've learned in the previous section how to create your own Wysiwyg parser. However, Front-Commerce connectors come with already implemented parsers. In this section, you will learn how they are configured and how to customize them.
...
However, in most cases, you will need to fetch data to display all the needed information in your widget. For instance, if the widget is `{% raw %}{{ widget type="acme/product-preview" sku="VSK12" }}{% endraw %}` you will want to fetch the product associated with the given SKU. This is the same concept as for the `WysiwygNodeData` but one level deeper in the GraphQL schema.
...
// my-module/server/modules/wysiwyg/index.js
contextEnhancer: ({ loaders }) => {
+  loaders.MagentoWidget.registerNodeType(
+    // the tag name in your HTML
...
This system should allow you to cover most of your use cases and we are very excited to see what you will build with it! If you have a new one though, feel free to contact us.
