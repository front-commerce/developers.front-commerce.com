...
One can force the contribution mode by using
[Front-Commerce's `contributionMode.force` configuration](/docs/3.x/api-reference/front-commerce-core/config#contributionmode).
In a default Front-Commerce application, this is achieved using the
`FRONT_COMMERCE_CONTRIBUTION_MODE_FORCE` environment variable in the
[`app/config/contributionMode.ts` configuration file](https://gitlab.blackswift.cloud/front-commerce/front-commerce/-/blob/update-contribution-mode-demos/skeleton/app/config/contributionMode.ts),
