...
It can be done from
[a GraphQL module `contextEnhancer`](/docs/2.x/reference/graphql-module-definition#contextenhancer-optional)
...
2. the **"Access token"** value must be the value of your
   ...
   ([also used for webhooks](/docs/2.x/prismic/installation#configure-the-environment-for-prismic))
...
[exposed content from the previously modified Prismic custom type in your GraphQL schema](/docs/2.x/prismic/expose-content).
...
- `entityName`: the name of the Sitemapable entity
  [used during sitemapable pages registration](/docs/2.x/advanced/production-ready/sitemap#add-dynamic-pages)
